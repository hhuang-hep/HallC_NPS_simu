    ...
    fTime_opening(-999.), fID_opening(-999)
{
  ...
  //G4cout<<"++++++++++ SteppintAction initialization done ++++++++++"<<G4endl; // added by Hao
}
...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  //G4cout<<"++++++++++ SteppingAction: get volume of the current step ++++++++++"<<G4endl; // added by Hao
  G4VPhysicalVolume* volume_pre = aStep->GetPreStepPoint()->GetTouchableHandle()->GetVolume();

  G4VPhysicalVolume* volume_post = aStep->GetPostStepPoint()->GetTouchableHandle()->GetVolume();
  
  //G4cout<<"++++++++++ SteppingAction: get tracks ++++++++++"<<G4endl; // added by Hao
  G4Track* track = aStep->GetTrack();
  //G4cout<<"particle ID = "<<aStep->GetTrack()->GetDynamicParticle()->GetParticleDefinition()->GetPDGEncoding()<<endl;
  //G4cout<<"Track PDG charge = "<<aStep->GetTrack()->GetDefinition()->GetPDGCharge()<<endl;
  //G4cout<<"++++++++++ SteppingAction: volume_pre is "<<volume_pre->GetLogicalVolume()->GetName()<<G4endl;
  if(
  ...
  if(
    ...
    //G4cout<<"++++++++++ SteppingAction: get a position of particle in local coordinate ++++++++++"<<G4endl; // added by Hao
  ...
  //G4cout<<"++++++++++ SteppingAction: collect deposited energy ++++++++++"<<G4endl; // added by Hao
  ...
  if(// This does not work that well.
     ...
     //     && volume_post->GetLogicalVolume()->GetName() == "Crystal_log"
      ...
      //G4cout<<"++++++++++ SteppingAction #1 ++++++++++"<<G4endl; // added by Hao
  ...
  if(
     ...
     // && volume_post->GetLogicalVolume()->GetName() != "Flux_log"
      ...
      //G4cout<<"++++++++++ SteppingAction #2 ++++++++++"<<G4endl; // added by Hao
  ...
  //G4cout<<"++++++++++ SteppingAction #3 ++++++++++"<<G4endl; // added by Hao
  
  G4double stepl = 0.;
  if (aStep->GetTrack()->GetDefinition()->GetPDGCharge() != 0.) stepl = aStep->GetStepLength();
  //G4cout<<"particle ID = "<<aStep->GetTrack()->GetDynamicParticle()->GetParticleDefinition()->GetPDGEncoding()<<endl;
  //G4cout<<"Track PDG charge = "<<aStep->GetTrack()->GetDefinition()->GetPDGCharge()<<endl;
  //G4cout<<"Volume_pre is "<<volume_pre->GetLogicalVolume()->GetName()<<G4endl;
  //G4cout<<"Step Length = "<<stepl<<endl;
  //G4cout<<"Particle position is "<<aStep->GetTrack()->GetPosition()<<" mm"<<G4endl;
  //G4cout<<" Next volume is "<<volume_pre->GetLogicalVolume()->GetName()<<G4endl;
  if (volume_pre->GetName() == "Target_log") fEventAction->AddAbs(edep,stepl);
  //G4cout<<"++++++++++ SteppingAction #5 ++++++++++"<<G4endl; // added by Hao
  //G4cout<<"Particle position is "<<aStep->GetTrack()->GetPosition()<<" mm"<<G4endl;
  if (volume_pre->GetName() == "Crystal_log")      fEventAction->AddGap(edep,stepl);
  //G4cout<<"Particle position is "<<aStep->GetTrack()->GetPosition()<<" mm"<<G4endl;
  //G4cout<<"Particle energy is "<<aStep->GetTrack()->GetTotalEnergy()<<" GeV"<<G4endl;
  //G4cout<<"Current step is "<<aStep->GetStep()<<G4endl;
  //G4cout<<"++++++++++ SteppingAction #6 (end) ++++++++++"<<G4endl; // added by Hao
}
  //....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
