    ...
    fPrintModulo(0)                             
{
    ...
    L_calo_phot1 = new TLorentzVector();//If I do not do it, it crashes.....
    L_calo_phot2 = new TLorentzVector();//If I do not do it, it crashes.....

    dvcs_evt->SetCaloEvent(calo_evt);
    G4cout<<"Calorimter distance (cm): "<<dvcs_evt->GetGeometry()->GetCaloDist()<<G4endl;
...
void EventAction::EndOfEventAction(const G4Event* evt)
{
    ...
    if( dvcsGlobals::hit_HMS_CALO_flag ){
        ...
        for(G4int i = 0 ; i < Nb_clust ; i++ ){
            ...
            // if(Nb_clust) fHistoManager->SetClusterPositionandMomentum(evt->GetEventID(), clus->GetX(), clus->GetY(), L_calo_phot->E(), L_calo_phot->Px(), L_calo_phot->Py(), L_calo_phot->Pz()); // Not in use
        }
        if(Nb_clust >= 2 && hit_HMS){
            // cluster of first photon
            G4double clust_x1   = calo_evt->GetCluster(0)->GetX();
            G4double clust_y1   = calo_evt->GetCluster(0)->GetY();
            G4double clust_ene1 = calo_evt->GetCluster(0)->GetE();
            G4int clust_size1   = calo_evt->GetCluster(0)->GetClusSize();
            fHistoManager->SetCaloData1(clust_ene1, clust_x1, clust_y1, clust_size1);
            // cluster of second photon
            G4double clust_x2   = calo_evt->GetCluster(1)->GetX();
            G4double clust_y2   = calo_evt->GetCluster(1)->GetY();
            G4double clust_ene2 = calo_evt->GetCluster(1)->GetE();
            G4int clust_size2   = calo_evt->GetCluster(1)->GetClusSize();
            fHistoManager->SetCaloData2(clust_ene2, clust_x2, clust_y2, clust_size2);

            // Photon1 momentum after clustering
            *L_calo_phot1 = dvcs_evt->GetPhoton(0);
            G4double phot_px1 = L_calo_phot1->Px();//GeV
            G4double phot_py1 = L_calo_phot1->Py();
            G4double phot_pz1 = L_calo_phot1->Pz();
            fHistoManager->SetPhotRec1(phot_px1, phot_py1, phot_pz1); //Set Reconstructed proton1 kinematics: Px, Py, Pz
            // Photon2 momentum after clustering
            *L_calo_phot2 = dvcs_evt->GetPhoton(1);
            G4double phot_px2 = L_calo_phot2->Px();//GeV
            G4double phot_py2 = L_calo_phot2->Py();
            G4double phot_pz2 = L_calo_phot2->Pz();
            fHistoManager->SetPhotRec2(phot_px2, phot_py2, phot_pz2); //Set Reconstructed proton2 kinematics: Px, Py, Pz

            fHistoManager->SetMpi0Rec((*L_calo_phot1+*L_calo_phot2).M());//Set Reconstructed pi0 mass
            ...
            TVector3 k(fPx[0], fPy[0], fPz[0]);
            TVector3 kp(fPx[5], fPy[5], fPz[5]);
            TVector3 qp1(phot_px1, phot_py1, phot_pz1);
            TVector3 qp2(phot_px2, phot_py2, phot_pz2);
            TVector3 qp = qp1+qp2;
            ...
            TLorentzVector Lb, Lp, Lgp1, Lgp2, Lkp;
            Lb.SetPxPyPzE(fPx[0], fPy[0], fPz[0], fE[0]);
            Lp.SetPxPyPzE(0, 0, 0, M_targ);
            Lkp.SetPxPyPzE(fPx[5], fPy[5], fPz[5], fE[5]);
            Lgp1 = *L_calo_phot1;
            Lgp2 = *L_calo_phot2;

            G4double mm2 = (Lb + Lp - Lgp1 - Lgp2 - Lkp).M2();
...
void EventAction::DefinePrimaries(TLorentzVector* L_elec_init, TLorentzVector* L_elec_scat, TLorentzVector* L_phot1_final, TLorentzVector* L_phot2_final, TLorentzVector* L_prot_final, G4double vertex_z)
{
    fPx[0] = L_elec_init->Px(); fPx[1] = L_elec_scat->Px(); fPx[2] = L_phot1_final->Px(); fPx[3] = L_phot2_final->Px(); fPx[4] = L_prot_final->Px();
    fPy[0] = L_elec_init->Py(); fPy[1] = L_elec_scat->Py(); fPy[2] = L_phot1_final->Py(); fPy[3] = L_phot2_final->Py(); fPy[4] = L_prot_final->Py();
    fPz[0] = L_elec_init->Pz(); fPz[1] = L_elec_scat->Pz(); fPz[2] = L_phot1_final->Pz(); fPz[3] = L_phot2_final->Pz(); fPz[4] = L_prot_final->Pz();
    fE[0]  = L_elec_init->E();  fE[1]  = L_elec_scat->E();  fE[2]  = L_phot1_final->E();  fE[3]  = L_phot2_final->E();  fE[4]  = L_prot_final->E();
    ...
    fHistoManager->SetInitElecGen(beam_px, beam_py, beam_pz);
    fHistoManager->SetInitElecRec(fPx[0], fPy[0], fPz[0]);
    fHistoManager->SetScatElecGen(fPx[1], fPy[1], fPz[1]);
    fHistoManager->SetPhotGen1(fPx[2], fPy[2], fPz[2]);
    fHistoManager->SetPhotGen2(fPx[3], fPy[3], fPz[3]);

    //===========Calculate Q2, t, phi from DVCS event generator=========

    TVector3 mom_init_elec_gen(0, 0, fE[0]);
...
void EventAction::DefineHMS_elec(G4ThreeVector mom, G4double ene)
{
    ...
    fHistoManager->SetScatElecRec(fPx[5], fPy[5], fPz[5]);
    hit_HMS = true;
    ...
    //HMS_angle
    G4int run_number = dvcsGlobals::run_number;
    G4double ry = -0.01*fSmeared_vertex_z*TMath::Sin(TMath::ATan2(fPx[5],fPz[5]));  // 0.01 to convert cm to meters
    G4double rdp= (sqrt(fPx[5]*fPx[5]+fPy[5]*fPy[5]+fPz[5]*fPz[5])-pcentral)/pcentral;
    G4double rtheta = TMath::ATan2(-fPy[5],TMath::Sqrt(fPx[5]*fPx[5]+fPz[5]*fPz[5]));
    G4double rphi   = TMath::ATan2(fPx[5],fPz[5]) - HMS_angle;
