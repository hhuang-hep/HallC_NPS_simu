...
gEv=new TGenElas(BeamEnergy,target_gen_proc_type,seed1,seed2);
...
//    gEv->Settmin(tmax - 2.); //Use this method to change tmin (default -2 GeV)                                                                                                     
    ...
    if(gEv->HitsSpectro(gEv->GetFinalProton()) && gEv->HitsCalo(gEv->GetScatteredElectron())){//Checks calo acceptance

      hit_HMS = true;

      L_elec_init  = gEv->GetInitialElectron();
      L_elec_scat  = gEv->GetScatteredElectron();
      L_phot_final = gEv->GetFinalProton();// In elastic kinematics, there is no photon. We assign the scattered proton to this variable.
	...
	fParticleGun->SetParticleDefinition(particleTable->FindParticle(2212));//2212 : proton
	fParticleGun->SetParticleEnergy(gEv->GetFinalProton()->E()*CLHEP::GeV- 0.938272088*CLHEP::GeV);
	fParticleGun->SetParticlePosition(G4ThreeVector(gEv->GetVertex()->X()*CLHEP::cm, gEv->GetVertex()->Y()*CLHEP::cm, gEv->GetVertex()->Z()*CLHEP::cm));
	fParticleGun->SetParticleMomentumDirection(G4ThreeVector((gEv->GetFinalProton()->Vect()).Px(), gEv->GetFinalProton()->Vect().Py(), gEv->GetFinalProton()->Vect().Pz()));
