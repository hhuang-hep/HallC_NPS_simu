...
PrimaryGeneratorAction::PrimaryGeneratorAction(EventAction* evt, long seed1, long seed2)
  : G4VUserPrimaryGeneratorAction(),
    ...
    M_targ(-999.),
    fParticleGun(0),
    L_elec_init(0), L_elec_scat(0), L_phot1_final(0), L_phot2_final(0), L_prot_final(0)
{
  ...
  gEv=new TGenPi0(BeamEnergy, target_gen_proc_type, seed1, seed2);//(beam-energy [GeV], target type, seed1, seed2)//But I do not think the seed1 and seed2 is actually used in TGenDVCS.
...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if(gEv->ComputeElectron()){//Check hor spectro accep.
    ...
    gEv->ComputePi0();//Computes the gamma* p -> pi0 p' reaction in the center of mass.
    ...
    if(gEv->HitsSpectro(gEv->GetScatteredElectron()) && gEv->HitsCalo(gEv->GetFinalPhoton1()) && gEv->HitsCalo(gEv->GetFinalPhoton2())){//Checks calo acceptance
        
        hit_HMS = true;

        L_elec_init  = gEv->GetInitialElectron();
        L_elec_scat  = gEv->GetScatteredElectron();
        L_phot1_final = gEv->GetFinalPhoton1(); 
        L_phot2_final = gEv->GetFinalPhoton2(); 
        ...
        if(hit_HMS){
	        ...
	        if(target_gen_proc_type == 0){//proton
	            ...
	            G4double X_sum  = gEv->XS_pi0();
	            G4double X_diff = 0;//gEv->XSecDif();
	            G4double X_BH   = 0;//gEv->XSecSum(1);
	        ...
	        fEventAction->DefinePrimaries(L_elec_init, L_elec_scat, L_phot1_final, L_phot2_final, L_prot_final, gEv->GetVertex()->Z());
            ...
            fParticleGun->SetParticleEnergy(gEv->GetFinalPhoton1()->E()*CLHEP::GeV);
            fParticleGun->SetParticlePosition(G4ThreeVector(gEv->GetVertex()->X()*CLHEP::cm, gEv->GetVertex()->Y()*CLHEP::cm, gEv->GetVertex()->Z()*CLHEP::cm));
            fParticleGun->SetParticleMomentumDirection(G4ThreeVector((gEv->GetFinalPhoton1()->Vect()).Px(), gEv->GetFinalPhoton1()->Vect().Py(), gEv->GetFinalPhoton1()->Vect().Pz()));

            fParticleGun->GeneratePrimaryVertex(anEvent);

            fParticleGun->SetParticleDefinition(particleTable->FindParticle(22));//22 : photon
            fParticleGun->SetParticleEnergy(gEv->GetFinalPhoton2()->E()*CLHEP::GeV);
            fParticleGun->SetParticlePosition(G4ThreeVector(gEv->GetVertex()->X()*CLHEP::cm, gEv->GetVertex()->Y()*CLHEP::cm, gEv->GetVertex()->Z()*CLHEP::cm));
            fParticleGun->SetParticleMomentumDirection(G4ThreeVector((gEv->GetFinalPhoton2()->Vect()).Px(), gEv->GetFinalPhoton2()->Vect().Py(), gEv->GetFinalPhoton2()->Vect().Pz()));
